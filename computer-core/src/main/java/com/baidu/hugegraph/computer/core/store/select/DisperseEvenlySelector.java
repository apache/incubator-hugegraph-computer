...
public class DisperseEvenlySelector implements InputFilesSelector {
    ...
    @Override
    public List<SelectedFiles> select(List<String> inputs,
        ...
        // Init outputs
        List<Node> outputNodes = new ArrayList<>(outputs.size());
        int i = 0;
        for (; i < outputs.size(); i++) {
            HgkvDir inputDir = inputDirs.get(i);
            Node outputNode = new Node(inputDir.numEntries(),
                                       Lists.newArrayList(inputDir.path()),
                                       outputs.get(i));
            outputNodes.add(outputNode);
        }
        // Distribute the remaining input
        Heap<Node> heap = new Heap<>(outputNodes,
