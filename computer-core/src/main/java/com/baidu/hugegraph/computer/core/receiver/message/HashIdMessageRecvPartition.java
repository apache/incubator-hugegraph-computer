...
public class HashIdMessageRecvPartition extends MessageRecvPartition {

    private static final String TYPE = MessageType.HASHID.name();

    private final OuterSortFlusher flusher;

    public HashIdMessageRecvPartition(ComputerContext context,
                                       SuperstepFileGenerator fileGenerator,
                                       SortManager sortManager) {
        super(context.config(), fileGenerator, sortManager, false);
        Config config = context.config();
        Combiner<Value<?>> combiner = null;
